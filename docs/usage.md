...
The default is `expax.normest.onenormest()`, whose `4 * block_size` model is approximately four batches of `block_size` parallel `matvec` applications, when the dimension permits it, and exact basis actions for dimensions one and two.
