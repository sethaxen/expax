...
to float64. A longer series must select the same float64 value before the table
is written.
"""
...
BACKWARD_ERROR_TRUNCATION = 2**10
BACKWARD_ERROR_VERIFICATION_TRUNCATION = BACKWARD_ERROR_TRUNCATION + 200
ROOT_ACCURACY_BITS = 2**7
ARB_GUARD_BITS = 2**7
TOLERANCE_EXPONENTS = (8, 11, 24, 53)
...
def downward_float64(lower: fmpq, upper: fmpq) -> float:
    """Return the greatest float64 proved to lie below the interval [lower, upper]."""
    ...
    if not _as_fmpq(candidate) <= lower < upper < _as_fmpq(successor):
        raise ArithmeticError("root bracket does not determine one float64 value")
    return candidate
...
def validate_majorants(
    ...
    """Require longer majorants to select the same downward float64 values."""
...
def compute_theta_values(
    ...
    with flint.ctx.workprec(ROOT_ACCURACY_BITS + ARB_GUARD_BITS):
        ...
        values = tuple(downward_float64(*bracket) for bracket in brackets)
...
def render_module(values: dict[float, tuple[float, ...]]) -> str:
    """Render the generated table with exact float64 decimal spellings."""
    lines = [
